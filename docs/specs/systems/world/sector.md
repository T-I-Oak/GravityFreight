...
- **`constructor(session: SessionState, isAnomaly: boolean)`**
    - **挙動**:
        ...
        2. **母星（Home Star）の配置**:
            ...
            - ※具体的な半径と質量は `CelestialBody` 内部で `GameDataRepository` から取得される。
        3. **天体数の決定**:
            - `GameDataRepository.getMasterConfig().baseCelestialCount` に `session.blackMarketVisits` を加算して決定する。
        4. **天体（CelestialBody）の配置生成**:
            ...
            - **座標決定**: 以下の制約を満たすワールド座標 `(x, y)` を最大 100 回試行して決定する。
                ...
                - **境界制約**: 原点からの距離が **700px 以下**（`GameDataRepository.getMasterConfig().placementLimitRadius` を参照）。
