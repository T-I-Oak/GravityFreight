...
- `useAvoidanceModule(type: 'body' | 'boundary', target: object | null): AvoidanceResult | null`
    ...
    - **内部挙動**:
        1. **回避可否の判定**:
            - **通常時**: 該当する通常モジュール、ゴースト系モジュールの順に検索する。`Charges > 0` の個体があれば **Charges を 1 減算** し、回避実行ステップへ進む。
            - **予測時（isGhost）**: 通常モジュールは無視し、該当するゴースト系モジュールを検索する。ゴースト系モジュールがあれば `Charges` の残量に関わらず回避実行ステップへ進み、`Charges` は減算しない。
            - ゴースト系モジュールは、対応能力込みの予測線を表示するだけでなく、実航行でも同じ対応能力を耐久消費で 1 回発動できる。
        2. **回避実行（共通処理）**:
