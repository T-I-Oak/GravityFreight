...
インスタンスからアクセス可能なプロパティ。強化対象の項目は強化回数に応じた補正後の値を返す。
...
### 3.1 耐久度 0 の扱い
charges が 0 の場合、mass を除くすべての性能プロパティは無効化される。
...
### 3.3 強化ルール
enhancement には強化回数を記録し、UI はこれに基づきハイライト表示等を行う。
