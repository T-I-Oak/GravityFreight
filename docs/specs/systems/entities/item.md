...
- **`consumeCharge(): boolean`**
    - 耐久度を 1 減らし、`charges <= 0` になった場合に `true`（破棄・消失）を返す。
    - **破棄の扱い**: `true` が返された場合、そのアイテムインスタンスは保持リスト（Rocket）やインベントリから即座に除外され、ゲームから消滅する。
...
- **`calculateAppraisalValue(): number`**
    - 自身の現在状態に基づく査定額を返す。
    - **計算**: `floor(レアリティ基本価格 * 耐久補正 * 強化補正)`。
    - 耐久補正は `maxCharges > 0` の場合 `(charges + 1) / (maxCharges + 1)`、耐久を持たないアイテムは `1` とする。
    - 強化補正は `1 + enhancementCount * 0.1` とする。
