...
- 永続化（保存）時には、再構築に必要な `uid`, `id`, `charges`, `maxCharges`, `enhancement` のみが必須となる。
- UI への受け渡し時には、マスタ参照を不要にするため全プロパティを含める。

#### static fromSnapshot(data)
スナップショットデータからインスタンスを再構築（Hydration）して返す。
- **処理**: `uid`, `masterId` で constructor を呼び出した後、`charges`, `maxCharges`, `enhancement` を反映し、各性能プロパティを再計算する。
