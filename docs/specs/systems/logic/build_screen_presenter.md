...
- **責務**:
    ...
    - `SessionState.returnBonus` がある場合、発射ボタンに現在の発射パワー倍率を表示する。
- **責務外**:
...
- `sessionState.returnBonus > 0` の場合、`launch.bonusText` は `build.launch.returnBonusText` に `1 + returnBonus` を小数第1位の倍率 (`x1.1`, `x1.2` など) として埋め込んだ文言にする。
- 帰還ボーナスは launch equipment の選択前でも確認できるように、`launch.ready` とは独立して表示データを生成する。
- 帰還ボーナスがない場合、`launch.bonusText` は空文字にする。
- `launch.subtext` は通常の ready / waiting 文言を使用する。
