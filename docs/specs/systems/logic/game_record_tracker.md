...
- **役割**: ゲーム全体の記録値管理。
- **責務**:
    - セクター開始、航行終了、施設取引、配達成功、ストーリー既読以外の契約終了結果から、累計値・最大値などの抽象的な記録値を更新する。
...
- `AchievementTracker` は `GameRecordTracker` が保持する現在の記録値を参照し、前回評価時との差分から新規に到達した tier を判定する。
