...
- 実績定義そのもの、表示ラベル、tier の `goal` / `title`、参照ソース、参照キー、最大値判定か最小値判定かの条件種別は `content_achievements.json` のマスタデータとして扱う。
- 実績定義は以下の形を基本とする。
```javascript
{
  id: string,
...
- `source: 'story_read'` の実績は `StorySystem.getReadCounts()` の集計値を参照する。ストーリー既読数実績はこのソースを使用する。
...
現在の `content_achievements.json` に存在する実績は、以下のタイミングで再評価する。
...
- `stat_stories_read` は通常39話と特殊ストーリー `HOME25` を合わせた全40話の既読数を参照する。
- `stat_*_branch` は通常ストーリーIDの先頭文字が対象系統に一致する件数を参照する。`HOME25` は T/R/B 系統別実績には含めない。
...
- `stat_max_dist` / `stat_distance` の「航行距離」は world px の移動量ではなく、航行 tick 数を記録値として扱う。
