...
- **責務**:
    ...
    - `BackgroundManager` による遠景描画、`SectorMapRenderer` によるセクター内マップ描画、`FlightVisualRenderer` による航行ビジュアル描画の順序を統合する。
...
- **`soundController: SoundController | null`**: ワープ演出と同期する環境音の制御先。ワープ音の開始・停止はワープ描画の開始・停止と同じ入口で扱う。
...
- **`setSoundController(soundController: SoundController | null): void`**
    - App 初期化後に音響システムを紐付ける。
    - `WorldRenderer` がワープ演出の開始・終了を受けたタイミングで、対応するワープ環境音も開始・停止するために使用する。
...
- **`setAimRocket(rocket: Rocket): void`**
    ...
    - **exit arc プレビュー**: AIM 中は `aimRocket.getArcMultiplier()` を exit arc の描画幅に反映し、アーク・エクスパンダー等による有効出口幅をプレビューする。
...
- **`startWarpEffect(duration: number, options?: { direction?: 'forward' | 'reverse' }): void`**
    - ワープ演出を開始する。
    - `direction: 'forward'` または省略時:
        - 現在のセクターから離脱し、次セクター方向のワープ空間へ突入する。
        - 背景の星の流速は `BackgroundManager.startWarpEffect(duration)` が管理する。
        - セクター内オブジェクトは Canvas 中心を基準に Scale 1.0→100.0、Alpha 1.0→0.0 へ補間する。
    - `direction: 'reverse'`:
        - ゲームオーバー時に現在セクターからズームアウトし、スタートのセクター方向へ戻るように見せる。
        - 背景の星は `BackgroundManager.startReverseWarpEffect(duration)` で逆方向に流す。
        - マップは縮退し、一定サイズ以下では中央に残り続けないよう透明化する。
    - どちらの方向でも、同じタイミングで `SoundController.startWarpEffect(duration / 1000, { direction })` を呼び出し、背景・マップ・ワープ音を同期する。

- **`stopWarpEffect(duration: number, options?: { fromCurrent?: boolean }): void`**
    - ワープを終了し、新しいセクターの入り口に到達した際の演出を終了する。
    - 背景の星の流速は `BackgroundManager` が通常状態へ戻す。
    - `fromCurrent` が未指定の場合、セクター内オブジェクトは Canvas 中心を基準に Scale 0.01→1.0、Alpha 1.0 のまま補間する。
    - `fromCurrent` が `true` の場合、現在の `mapWarp` 状態から通常表示へ戻す。ゲームオーバー演出からタイトルへ戻る際に使用する。
    - 同じタイミングで `SoundController.stopWarpEffect(duration / 1000)` を呼び出し、ワープ音を自然に停止する。

- **`resetMapWarp(): void`**
    - `mapWarp` の scale / alpha / transition / fadeMode を通常表示へ即時リセットする。
    - リプレイ開始のように、前のゲームオーバー逆ワープ状態を引き継いではいけない画面切替で呼び出す。
    - カメラ位置、ズーム、回転は `CameraController` の責務であり、このメソッドでは変更しない。

- **ゲームオーバー逆ワープ**
    - ゲームオーバー時は `startWarpEffect(duration, { direction: 'reverse' })` を呼び出す。
    - 通常ワープは前セクターのマップへズームインして次セクター方向へ進むが、ゲームオーバー演出では現在マップからズームアウトして、スタートのセクター方向へ戻るように見せる。
    - マップは一定サイズまで縮退した後、中央に残り続けないように透明度を下げて消す。
    - `END CONTRACT` 押下後は `stopWarpEffect(duration, { fromCurrent: true })` を呼び出し、現在の退場ワープ状態からタイトル復帰可能な通常背景状態へ戻す。
...
セクター内の各データオブジェクトは `SectorMapRenderer` が Canvas 2D の描画命令として描画する。`WorldRenderer` は transform / view / colors / activeRocket を渡し、描画順序を管理する。
...
- セクター内の天体に、その施設を `deliveryGoalId` とする配送対象 `cargo` が存在する場合、arc 外側に cargo アイコンを描画する。`deliveryGoalId` を持たない特殊 cargo は配送対象ではないため、このアイコンの対象外とする。
    ...
    - アイコン形状は ItemCard の配送先 badge と同じ共有定義を参照する。
