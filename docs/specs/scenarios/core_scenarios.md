...
- [ ] 8.2 [機能] 退場演出（逆方向ワープ等）
    ...
    - **ゲームリザルトの重ね表示**:
        ...
        - ゲームリザルトには累計スコア、総獲得コイン、到達セクター数、合計航行時間など、7.1で定義した最終結果を表示する。
    ...
    - **タイトル復帰**:
        ...
        - タイトルへ戻る際、現在の `SessionState` は破棄される。ただし、記録・実績・リプレイなど App Lifecycle の永続データは、リザルト表示前または表示時点で確定済みであること。
...
- [ ] 9.2 [機能] エンジンクラスを利用した操作デモ
    ...
    - **使用するクラス/責務**:
        ...
        - `DataManager`: 説明書内に表示するアイテムカード、施設名、カラー、定数を取得する。
    - **停止条件**:
...
- [ ] 10.1 [機能] 過去の航行成績・実績の閲覧
    - [ ] 10.1.1 [設計] 閲覧対象とする統計項目（累計額、距離等）の定義
        ...
        - **分析表示候補**:
            ...
            - 航行ごとの到達セクター数推移
            - 航行ごとの回収アイテム数推移
            - Personal Best Ranking（スコア、セクター、回収数など）
        - **データソース**:
            - 累計統計と実績進捗は `AchievementTracker` または実績用永続データから取得する。
            - 航行単位の推移・ランキングは `FlightRecorder` が保存する航行記録から取得する。
    ...
    - [ ] 10.1.3 [UI] 統計・実績リストの表示レイアウト
        ...
        - **Replays タブ**:
            ...
            - ロケット、発射台、ブースターなどの発射構成は、一覧で概要表示するか、リプレイ再生時の構成パネルで表示する。最終仕様は 10.2 の再生方式と合わせて確定する。
- [ ] 10.2 [機能] リプレイ再生の管理
    - **保存単位**:
        - リプレイは1回の航行単位で保存する。
        - 保存対象には、その航行の初期状態、発射構成、発射角度、セクター状態、物理再生に必要なデータ、航行結果を含める。
    - **自動保存ルール**:
        - 1回の航行で取得したスコアを基準に、スコア上位の航行記録を自動保存する。
        - 自動保存枠の上限数は最大15件とする。
        - 自動保存枠が上限に達した状態で新しい高スコア航行が発生した場合、スコア下位かつお気に入りではない記録から置き換える。
    - **お気に入り登録**:
    ...
    - **リプレイ一覧の表示内容**:
        ...
        - 記録日付
        - お気に入り状態
        - 発射構成（RocketItem / Launcher / Booster）の概要表示は任意。ただし、再生中には構成を確認できること。
    - **再生画面**:
        - リプレイ再生は全画面で行う。
        - 通常プレイと同じマップ表示を使い、航行軌道、重力干渉、到達/失敗の流れを通常プレイと同じ空間スケールで確認できるようにする。
        - 右側のビルドパネル領域には、発射構成（RocketItem / Launcher / Booster）や航行メタ情報を表示する。
    - **再生中の構成表示**:
        - 通常プレイ画面の右側ビルドパネル相当の領域を、リプレイ中は構成表示パネルとして使用する。
        - 構成表示パネルには、使用した RocketItem、Launcher、Booster、主要性能、発射角度、航行結果を表示する。
