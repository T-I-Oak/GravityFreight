...
- **クラス**: `.ui-durability-gauge` / `.ui-durability-segment`
- **基本形状**: 
    - **常用サイズ**: 幅10px、高さ5px。
...
- **修飾クラス (Modifier Classes)**:
    ...
    - **実装上の注意**: ベースの `.ui-badge` が持つ `min-height` 制約を確実に解除し、10px の高さを保証するため、修飾クラスによるジオメトリ変数の上書き（`--badge-min-height: 0`）が適用されている。
...
- **`charges` (number, optional)**: 現在の耐久度（セグメント点灯数）。
- **`maxCharges` (number, optional)**: 最大耐久度（セグメント総数）。
- **`isEnhanced` (boolean, optional)**: 強化状態であることを示す特殊エフェクト（外枠の発光等）を表示するかどうか。

### 5.3 性能パラメータ (Stats)
- **`stats` (Object, optional)**: 性能パラメータの集合。UI はこの中から必要なキーをピックアップして描画する。
    - **各項目の構造**: `{ value: number | string, isEnhanced: boolean }`
    - **主要なキー例**: `mass`, `slots`, `power`, `precisionMultiplier`, `pickupMultiplier`, `gravityMultiplier` 等。
