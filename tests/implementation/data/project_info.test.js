...
describe('project_info.json', () => {
    ...
    it('keeps non-localized portal metadata as plain values', () => {
        ...
        expect(projectInfo.button.type).toBe('pending');
        expect(projectInfo.image).toBe('assets/thumbnail.png');
    });

    it('uses the current portal badge wording and featured period', () => {
        expect(projectInfo.badge.content['lang-store'].ja).toBe('鋭意作成中！');
        expect(projectInfo.badge.content['lang-store'].en).toBe('In active development!');
        expect(projectInfo.badge.featuredUntil).toBe('2026-06-30');
    });
});
